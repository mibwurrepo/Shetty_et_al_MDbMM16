...
Plot presence/absence of taxa at each of the sampling timpoints in the three bioreactors.  
...
### Major SCFAs  
```{r hplc, fig.align='center', fig.height=6, fig.width=8, fig.cap='SCFA production in MDb-MM'}
...
ind_tax_plot + hplc.plot +
  ...
  plot_annotation(tag_levels = "A") & theme(legend.text = element_text(size=14))
ggsave("ms/figs/01_ind_tax_hplc.pdf", height = 6, width = 18)
```
...
biom.plot <- ggviolin(od, "Condition", "OpticalDensity",
  ...
  theme_syncom() + ylim(1,12) + ylab(expression(O.D[600~nm])) + xlab(NULL) + 
...
ggsave("ms/figs/01_supp_od_qpcr_gini_cov.pdf", height = 6, width = 10)
ggsave("ms/figs/01_supp_od_qpcr_gini_cov.png", height = 6, width = 10, dpi=600)
```
...
hplc.148.plot / hplc.152.plot + plot_layout(guides = "collect", heights = c(1,2)) + 
  plot_annotation(tag_levels = "A")
ggsave("ms/figs/02a_supp_scfa_subset.pdf", height = 8, width = 8)
ggsave("ms/figs/02a_supp_scfa_subset.png", height = 8, width = 8, dpi=600)
```
